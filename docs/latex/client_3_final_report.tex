...
\midrule
Total ranked questions & 52 & $\operatorname{count}(\text{all rows}) = 52$ & Starting universe for the refreshed package. \\
Selected scored questions & 28 & $\operatorname{count}(\texttt{selected\_for\_module}=\texttt{True}) = 28$ & Defines the scored module retained for auditability and review. \\
Deployable questions & 26 & $28 - 2 = 26$ after removing the selected demographic-only traceability rows & Defines the routine field instrument; assumes demographic context can be asked elsewhere or only when needed. \\
Selected scored minutes & 29.98 & $\sum \text{minutes across the 28 selected rows} = 29.98$ & Confirms the analytic shortlist stays inside the 30-minute review budget. \\
Deployable minutes & 28.82 & $29.98 - 0.35 - 0.817 = 28.813 \approx 28.82$ & Gives the paper-length upper bound for the deployable package. \\
Generic rows & 3 & $\operatorname{count}(\text{ri\_threshold\_score} \geq 0.70) = 3$ after normalization & Keeps only the strongest portable questions in the universal core. \\
Specific rows & 49 & $52 - 3 = 49$ & Preserves context-dependent detail inside one client-facing category. \\
Average $R_i$ & 2.201 & $\operatorname{mean}(R_i) = 2.201$ & Benchmark statistic showing average signal efficiency. \\
Maximum $R_i$ & 5.667 & $\max(R_i) = 5.667$ & Top efficiency value used in normalization. \\
Average $P_i$ & 2.561 & $\operatorname{mean}(P_i) = 2.561$ & Benchmark for distinctiveness-adjusted priority. \\
Maximum $P_i$ & 7.860 & $\max(P_i) = 7.860$ & Ceiling after rarity, construct, portability, and redundancy adjustments. \\
\bottomrule
\end{tabularx}
...
\filepath{deliverables/Master_Questionnaire.xlsx} & Full internal review sheet with traceability fields. \\
\filepath{deliverables/Deployable_Questionnaire.pdf} & Final deployable PDF questionnaire. \\
\filepath{deliverables/Codebook.pdf} & Rendered technical codebook PDF from the artifact pipeline. \\
\filepath{deliverables/Final_Report.pdf} & Rendered stakeholder-facing PDF from the artifact pipeline. \\
\filepath{deliverables/Client_1_Technical_Codebook.pdf} & LaTeX report version of the technical codebook. \\
\filepath{deliverables/Client_2_Deployable_Questionnaire.pdf} & LaTeX report version of the deployable questionnaire. \\
\filepath{deliverables/Client_3_Final_Report.pdf} & LaTeX report version of this stakeholder report. \\
\bottomrule
\end{tabularx}
